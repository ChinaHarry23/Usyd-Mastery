(function () {
  ...
  function getStoredChapterData() {
    ...
    try {
      var raw = localStorage.getItem("comp5318_progress");
      if (!raw) return {};
  ...
  function updateProgress() {
    ...
    var qs = 0;
    var qm = 0;
    if (typeof window.getComp5318ChapterProgress === "function") {
      var pr = window.getComp5318ChapterProgress();
      qs = pr.quickChecksStudy || 0;
      qm = pr.quickChecksMath || 0;
    }
    var pct;
    var label;
    if (qcDenom > 0) {
  ...
  bindDynamicContent();
  updateProgress();
  window.addEventListener("comp5318-progress-update", updateProgress);
  window.addEventListener("langchange", updateProgress);
  window.addEventListener("comp5318-content-refresh", bindDynamicContent);
