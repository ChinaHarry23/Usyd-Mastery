...
var ALL_FLASHCARD_DATA = [
  ...
  { ch: 1, front: "What are the six phases of CRISP-DM?", back: "Business Understanding → Data Understanding → Data Preparation → Modelling → Evaluation → Deployment.", front_zh: "CRISP-DM 的六个阶段？", back_zh: "业务理解→数据理解→数据准备→建模→评估→部署。" },
  { ch: 1, front: "Supervised vs unsupervised learning.", back: "Supervised uses labelled data (inputs→outputs); unsupervised finds structure in unlabelled data (e.g. clustering, PCA).", front_zh: "有监督与无监督学习的区别？", back_zh: "有监督使用标记数据（输入→输出）；无监督从无标签数据中发现结构（如聚类、PCA）。" },
  ...
  { ch: 2, front: "Min-max normalisation formula.", back: "\\(x' = (x - x_{\\min})/(x_{\\max} - x_{\\min})\\), maps each feature to [0, 1].", front_zh: "最小-最大归一化公式？", back_zh: "\\(x' = (x - x_{\\min})/(x_{\\max} - x_{\\min})\\)，将每个特征映射到 [0,1]。" },
  { ch: 2, front: "Z-score normalisation formula.", back: "\\(x' = (x - \\mu)/\\sigma\\); centres data at 0 with unit standard deviation.", front_zh: "Z-score 归一化公式？", back_zh: "\\(x' = (x - \\mu)/\\sigma\\)；使数据以 0 为中心、标准差为 1。" },
  ...
  { ch: 3, front: "Normal equation for OLS.", back: "\\(w^* = (X^\\top X)^{-1} X^\\top y\\); closed-form solution when \\(X^\\top X\\) is invertible.", front_zh: "OLS 的正规方程？", back_zh: "\\(w^* = (X^\\top X)^{-1} X^\\top y\\)；当 \\(X^\\top X\\) 可逆时的闭式解。" },
  { ch: 3, front: "Gradient descent intuition.", back: "Iteratively update \\(w \\leftarrow w - \\eta \\nabla L\\); move in the steepest descent direction scaled by learning rate \\(\\eta\\).", front_zh: "梯度下降直觉？", back_zh: "迭代更新 \\(w \\leftarrow w - \\eta \\nabla L\\)；沿最速下降方向移动，步长由学习率 \\(\\eta\\) 控制。" },
  ...
  { ch: 4, front: "Laplace smoothing in Naive Bayes.", back: "Adds a small count (usually 1) to each feature-class count to prevent zero probabilities for unseen values.", front_zh: "朴素贝叶斯的拉普拉斯平滑？", back_zh: "对每个特征-类别计数加小量（通常为 1），防止未见值产生零概率。" },
  { ch: 4, front: "Stratified k-fold cross-validation.", back: "Each fold preserves the overall class distribution, crucial for imbalanced datasets.", front_zh: "分层 k 折交叉验证？", back_zh: "每折保持整体类别分布比例，对不平衡数据尤为关键。" },
  ...
  { ch: 5, front: "Information gain for tree splitting.", back: "IG = parent entropy − weighted sum of children's entropy; split with highest IG is chosen.", front_zh: "决策树划分的信息增益？", back_zh: "IG = 父节点熵 − 子节点熵的加权和；选 IG 最大的划分。" },
  { ch: 5, front: "AdaBoost key idea.", back: "Sequentially train weak learners; up-weight misclassified samples so the next learner focuses on hard cases; final prediction is weighted vote.", front_zh: "AdaBoost 核心思想？", back_zh: "依次训练弱学习器；提高误分样本权重让下一个学习器关注难例；最终加权投票。" },
  ...
  { ch: 6, front: "Support vectors in SVM.", back: "Training points lying on or within the margin boundary; they alone determine the decision hyperplane.", front_zh: "SVM 中的支持向量？", back_zh: "位于间隔边界上或以内的训练点；仅它们决定决策超平面。" },
  { ch: 6, front: "Scree plot in PCA.", back: "Plots eigenvalues (or explained variance) vs component index; the 'elbow' suggests how many components to retain.", front_zh: "PCA 的碎石图？", back_zh: "以特征值（或解释方差）对分量序号作图；\"肘部\"暗示应保留多少分量。" },

  // —— Ch7 ——
  { ch: 7, front: "Perceptron output rule.", back: "\\(a = \\text{step}(w \\cdot x + b)\\); 1 if the linear score is non-negative, else 0.", front_zh: "感知机输出规则？", back_zh: "\\(a = \\text{step}(w \\cdot x + b)\\)；线性分非负为 1，否则为 0。" },
  { ch: 7, front: "Perceptron learning rule (matrix form).", back: "With error \\(e = t - a\\): \\(w \\leftarrow w + e\\,x^\\top\\), \\(b \\leftarrow b + e\\).", front_zh: "感知机学习规则（矩阵形式）？", back_zh: "设 \\(e = t - a\\)：\\(w \\leftarrow w + e\\,x^\\top\\)，\\(b \\leftarrow b + e\\)。" },
  { ch: 7, front: "Which gates a single perceptron can solve.", back: "AND, OR, NAND — all linearly separable. XOR cannot be expressed by one perceptron.", front_zh: "单层感知机能实现哪些门？", back_zh: "AND/OR/NAND（线性可分）；XOR 无法用单层感知机表达。" },
  { ch: 7, front: "Sigmoid and its derivative.", back: "\\(\\sigma(z) = 1/(1+e^{-z})\\); \\(\\sigma'(z) = \\sigma(z)(1-\\sigma(z))\\).", front_zh: "Sigmoid 及其导数？", back_zh: "\\(\\sigma(z) = 1/(1+e^{-z})\\)；\\(\\sigma'(z) = \\sigma(z)(1-\\sigma(z))\\)。" },
  { ch: 7, front: "Backprop output-layer delta.", back: "\\(\\delta_q = (t_q - o_q)\\,o_q\\,(1-o_q)\\) for a sigmoid output neuron.", front_zh: "反向传播输出层 δ？", back_zh: "Sigmoid 输出层：\\(\\delta_q = (t_q - o_q)\\,o_q\\,(1-o_q)\\)。" },
  { ch: 7, front: "Backprop hidden-layer delta.", back: "\\(\\delta_q = o_q\\,(1-o_q)\\,\\sum_i w_{qi}\\,\\delta_i\\) summed over downstream neurons \\(i\\).", front_zh: "反向传播隐藏层 δ？", back_zh: "\\(\\delta_q = o_q\\,(1-o_q)\\,\\sum_i w_{qi}\\,\\delta_i\\)，对下一层神经元求和。" },
  { ch: 7, front: "Weight update with learning rate.", back: "\\(w_{pq}^{\\text{new}} = w_{pq}^{\\text{old}} + \\eta\\,\\delta_q\\,o_p\\); bias updates as \\(\\theta_q \\leftarrow \\theta_q + \\eta\\,\\delta_q\\).", front_zh: "带学习率的权重更新？", back_zh: "\\(w_{pq}^{\\text{new}} = w_{pq}^{\\text{old}} + \\eta\\,\\delta_q\\,o_p\\)；偏置 \\(\\theta_q \\leftarrow \\theta_q + \\eta\\,\\delta_q\\)。" },
  { ch: 7, front: "Vanishing gradient (sigmoid).", back: "Sigmoid factor \\(o(1-o)\\) is at most 0.25 and near 0 in saturation; products through many layers collapse to near zero.", front_zh: "Sigmoid 的梯度消失？", back_zh: "Sigmoid 因子 \\(o(1-o)\\) 不大于 0.25，饱和时近 0；多层相乘趋零。" },
  { ch: 7, front: "ReLU and Leaky ReLU.", back: "ReLU: \\(\\max(0, x)\\). Leaky ReLU: \\(\\max(\\alpha x, x)\\) with small \\(\\alpha\\) to avoid dead units.", front_zh: "ReLU 与 Leaky ReLU？", back_zh: "ReLU：\\(\\max(0, x)\\)；Leaky ReLU：\\(\\max(\\alpha x, x)\\)，避免神经元「死亡」。" },
  { ch: 7, front: "Softmax + cross-entropy intuition.", back: "Softmax turns logits into probabilities summing to 1; cross-entropy \\(-\\sum_j y_j \\log \\hat y_j\\) heavily penalises confident wrong predictions.", front_zh: "Softmax + 交叉熵的直觉？", back_zh: "Softmax 把对数几率转为和为 1 的概率；交叉熵 \\(-\\sum_j y_j \\log \\hat y_j\\) 对自信的错误预测惩罚很重。" },

  // —— Ch8 ——
  { ch: 8, front: "What is a CNN filter / kernel?", back: "A small set of learnable weights (e.g. 3x3 or 5x5) convolved across the input to produce a feature map.", front_zh: "CNN 中的滤波器/卷积核？", back_zh: "一小组可学习权重（如 3x3、5x5），在输入上滑动卷积产生特征图。" },
  { ch: 8, front: "Conv output-size formula.", back: "\\(\\text{out} = \\lfloor (W - F + 2P)/S \\rfloor + 1\\) for input W, filter F, padding P, stride S.", front_zh: "卷积输出尺寸公式？", back_zh: "\\(\\text{out} = \\lfloor (W - F + 2P)/S \\rfloor + 1\\)；W 输入、F 滤波器、P 填充、S 步长。" },
  { ch: 8, front: "Max pooling vs average pooling.", back: "Max takes the largest value in a window (sharpest activation); average takes the mean (smoother, less common in modern CNNs).", front_zh: "最大池化 vs 平均池化？", back_zh: "最大池化取窗口内最大值（最强激活）；平均池化取均值（更平滑，现代 CNN 较少用）。" },
  { ch: 8, front: "Why weight sharing in CNNs?", back: "Same filter sliding across all positions: fewer parameters and translation-equivariant feature detection.", front_zh: "CNN 为何权重共享？", back_zh: "同一滤波器在所有位置共享：参数更少，且对平移具备等变性。" },
  { ch: 8, front: "Why an RNN, not an MLP, for sequences?", back: "RNN's hidden state carries information from previous time-steps; an MLP processes inputs independently.", front_zh: "序列任务为何用 RNN 而非 MLP？", back_zh: "RNN 的隐藏状态在时间步间传递信息；MLP 独立处理输入。" },
  { ch: 8, front: "BPTT in one sentence.", back: "Unroll the RNN T steps and run standard backprop on the resulting feedforward graph with shared weights.", front_zh: "一句话说 BPTT？", back_zh: "将 RNN 展开 T 步，在共享权重的前馈图上做标准反向传播。" },
  { ch: 8, front: "LSTM forget gate role.", back: "Sigmoid in [0,1] multiplied element-wise with previous cell state; chooses what to keep vs forget.", front_zh: "LSTM 遗忘门的作用？", back_zh: "范围 [0,1] 的 sigmoid，与上一细胞状态逐元素相乘，决定保留/遗忘。" },
